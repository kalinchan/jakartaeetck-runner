if [ ! -f jakartaeetck.zip ]; then
  wget --read-timeout=20 https://download.eclipse.org/ee4j/jakartaee-tck/jakartaee10/staged/eftl/jakarta-core-profile-tck-10.0.0-dist.zip -O jakartaeetck.zip
fi
if [ ! -f cdi-tck-3.0.3-dist.zip ]; then
  wget https://download.eclipse.org/ee4j/cdi/4.0/cdi-tck-4.0.1-dist.zip -O cdi-tck-4.0.1-dist.zip
fi
